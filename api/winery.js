...
// Old slugs that no longer match any wineries row — either the original seed
// migration's regex stripped accented characters (é, â, …) into stray hyphens
// instead of transliterating them, or a winery's slug was later renamed/
// corrected/merged. Google indexed these before the fix, so once a lookup by
// the incoming slug comes back empty, permanently redirect to the current
// slug instead of 404ing on an old indexed URL. Exported so it's inspectable
// (e.g. from a test) without duplicating the map.
const LEGACY_SLUGS = {
  'st-sup-ry': 'st-supery-estate',
  'st-sup-ry-estate': 'st-supery-estate',
  'ch-teau-sainte-marguerite': 'chateau-sainte-marguerite',
  'it-winery': 'ita-winery',
  'cobden-wini-wines': 'cobden-wini',
  'goldeneye-winery': 'goldeneye',
  'shannon-wine': 'shannon-family-of-wines',
  'lede-family-wines': 'clif-lede-vineyards',
  'grieve-winery': 'grieve-family-winery',
...
// exists. Accepted values come from lib/regions.js — the single shared
// source also used by api/region.js — so the two can't drift apart again.
const REGION_PAGE = (() => {
  const map = {};
  for (const [slug, group] of Object.entries(REGION_GROUPS)) {
    group.values.forEach(v => { map[v] = { slug, title: group.title }; });
  }
  return map;
})();
  ...
  try {
    ...
    if (wineryData.length === 0 && promoData.length === 0) {
      if (LEGACY_SLUGS[slug]) {
        res.writeHead(301, { Location: '/winery.html?slug=' + LEGACY_SLUGS[slug] });
        return res.end();
      }
...
module.exports.LEGACY_SLUGS = LEGACY_SLUGS;
